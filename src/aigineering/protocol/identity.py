"""Typed canonical identity for content-addressed protocol objects."""
...
def contract_from_fields(**fields: Any) -> Contract:
    """Construct a content-addressed Contract using the current identity rules.

    Minting-authority entries may contain ``CONTRACT_SELF_REFERENCE``.  The
    placeholder participates in the canonical hash and is expanded only in
    the materialized Contract, matching identity validation semantics.
    """
    from aigineering.protocol.types import Contract

    identity = hash_contract_current(**fields)
    materialized = dict(fields)
    if "minting_authority" in materialized:
        materialized["minting_authority"] = tuple(
            value.replace(CONTRACT_SELF_REFERENCE, identity)
            for value in materialized["minting_authority"]
        )
    return Contract(id=identity, **materialized)
